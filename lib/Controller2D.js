...
class Controller2D extends Model2D {
            ...
            for (let condition of action.conditions) {
                ...
                if (condition.when.index) {
                    ...
                    if (condition.when.index.row) {
                        if (/\sis\s/.test(condition.when.index.row)) {
                            const [index, type] = condition.when.index.row.split(/\s+is\s+/)
                            // console.log(this.item2D.index2D)
                            const row = new Function("index2D",
    ...
    async next({ action, condition, currentItem2D, checkItem2D, index2D }) {
        ...
        if (condition.next.state) {
            currentItem2D.state = condition.next.state
            success = true
        }
        if (condition.next.index) {
            if (condition.next.index.row) {
                const row = new Function("index2D",
                    `return ${condition.next.index.row.replace(/@/g, "index2D.")}`
                )(index2D)
                const nextIndex2D = {
                    ...index2D,
                    row
...
Controller2D.new = config => new Controller2D(config)
