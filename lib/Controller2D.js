...
class Controller2D extends Model2D {
                ...
                if (condition.when.index) {
                    ...
                    if (condition.when.index.column) {
                        if (/\sis\s/.test(condition.when.index.column)) {
                            ...
                            const nextItem2D = this.item2D.matrix2D.getItem2D(checkIndex2D)
                            console.log(nextItem2D)
                            if (!nextItem2D) {
                                console.log(`INVALID INDEX`, checkIndex2D)
                                continue
                            }
                            if (nextItem2D.name === type) {
                                await this.next({
                                    action: name,
                                    condition,
    ...
    async next({ action, condition, currentItem2D, nextItem2D, index2D }) {
